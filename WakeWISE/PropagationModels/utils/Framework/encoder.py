...
from WakeWISE.PropagationModels.utils.Framework.basics import MLP
...
class Encoder(nn.Module):
    ...
    def forward(self, node_attr, edge_attr, global_attr, batch):
        ...
        if node_attr is not None:
            if batch is None:
                node_attr = torch.cat([node_attr, global_attr.repeat(node_attr.shape[1], 1).repeat(node_attr.shape[0], 1, 1)], dim=2)
                node_attr_enc = self.node_encoder(node_attr)
            else:
